...
int isLabelAcceptable(int label, const int* acceptableLabels, int numAcceptableLabels) {
    // Check if the label is in the list of acceptable labels
    for (int i = 0; i < numAcceptableLabels; ++i) {
        if (label == acceptableLabels[i]) {
            return 1; // true
        }
    }
    return 0; // false
...
struct Point* readPointsFromFile(const char* filename, int* numPoints, const int* acceptableLabels, int numAcceptableLabels) {
    ...
    float x, y;
    int ignoredValue, label;

    // Count the number of lines in the file that have acceptable labels
    *numPoints = 0;
    while (fscanf(file, "%f %f %d", &x, &y, &label) == 3) {
        // Check if the label is in the list of acceptable labels
        if (isLabelAcceptable(label, acceptableLabels, numAcceptableLabels)) {
            (*numPoints)++;
        }
    }
    ...
    // Read points from the file, only including those with acceptable labels
    int currentIndex = 0;
    for (int i = 0; i < *numPoints; ++i) {
        fscanf(file, "%f %f %d", &x, &y, &label);

        // Check if the label is in the list of acceptable labels
        if (isLabelAcceptable(label, acceptableLabels, numAcceptableLabels)) {
            points[currentIndex].x = x * 1000;
            points[currentIndex].y = y * 1000;
            points[currentIndex].visited = 0; // false
            points[currentIndex].clusterId = -1;
            points[currentIndex].type = -1;
            points[currentIndex].label = label;
            currentIndex++;
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    const char *filename = "query_file (1).txt";
    ...
    const int acceptableLabels[] = {24, 45, 56};  // Replace with your acceptable labels
    int numAcceptableLabels = sizeof(acceptableLabels) / sizeof(acceptableLabels[0]);

    struct Point *h_points = readPointsFromFile(filename, &numPoints, acceptableLabels, numAcceptableLabels);
